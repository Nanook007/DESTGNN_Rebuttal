...
from torch.optim.lr_scheduler import MultiStepLR, CosineAnnealingLR, ReduceLROnPlateau, ExponentialLR


class ExLR(ExponentialLR):
    def __init__(self, optimizer, gamma, last_epoch=-1, verbose=False, min_lr=1.0e-6):
        self.gamma = gamma
        self.min_lr = min_lr
        super(ExponentialLR, self).__init__(optimizer, last_epoch, verbose)

    def get_lr(self):
        lr_candidate = super(ExLR, self).get_lr()
...
class COSLR2(CosineAnnealingLR):
    def __init__(self, optimizer, T_max, last_epoch=-1, verbose=False, min_lr=1.0e-6):
    ...
    def get_lr(self):
        lr_candidate = super(COSLR2, self).get_lr()
        if isinstance(lr_candidate, list):
            for i in range(len(lr_candidate)):
                lr_candidate[i] = max(self.min_lr, lr_candidate[i])
        else:
...
class trainer():
    def __init__(self, scaler, model, **optim_args):
        ...
        self.exp_gamma    = optim_args['exp_gamma']
        ...
        self.lr_decay_ways      = optim_args['lr_decay_ways']
        # curriculum learning
        ...
        if self.if_lr_scheduler:
            if self.lr_decay_ways == 'ExLR':
                self.lr_scheduler = ExLR(optimizer=self.optimizer, gamma=self.exp_gamma, verbose=False, min_lr=1.0e-07)
            if self.lr_decay_ways == 'cos':
                self.lr_scheduler = COSLR2(optimizer=self.optimizer, T_max=10, min_lr=1.0e-07)
            if self.lr_decay_ways == 'step':
                self.lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=self.lr_sche_steps, gamma=self.lr_decay_ratio)
    ...
    def set_resume_lr_and_cl(self, epoch_num, batch_num):
        if batch_num == 0:
            ...
            if self.if_cl:
                ...
                self.cl_len = self.output_seq_len
            print("resume training from epoch:{}, where learn_rate=:{} and curriculum learning length=:{}".format(epoch_num, self.lrate, self.cl_len))
        ...
        if self.print_model_structure and int(kwargs['batch_num'])==0:
            ...
            for name, param in self.model.named_parameters():
                ...
                for _ in param.shape:
                    tmp = tmp*_
                parameter_num += tmp
            print("Parameter size: {}".format(parameter_num))
        ...
        if self.if_cl:
            ...
            if kwargs['batch_num'] < self.warm_steps:   # warmupping
                ...
                if (kwargs['batch_num'] - self.warm_steps) % self.cl_steps == 0 and self.cl_len < self.output_seq_len:
                    self.cl_len += int(self.if_cl)
                    print("curriculum learning:{}".format(self.cl_len))
        else:
    ...
    @staticmethod
    def test(model, save_path_resume, device, dataloader, scaler, model_name, save=False, **kwargs):
        # test
        model.eval()
        outputs = []
