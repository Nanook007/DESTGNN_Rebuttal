...
def load_dataset(data_dir, batch_size, valid_batch_size, test_batch_size, dataset_name):
    ...
    for mode in ['train', 'val', 'test']:
        cat_data = np.load(os.path.join(data_dir, mode + '.npz'))
        data_dict['x_' + mode] = cat_data['x']
        data_dict['y_' + mode] = cat_data['y']

        # data_dict['x_' + mode] = cat_data['x'][:batch_size]
        # data_dict['y_' + mode] = cat_data['y'][:batch_size]
...
# def load_adj_PEMS(pkl_filename):
